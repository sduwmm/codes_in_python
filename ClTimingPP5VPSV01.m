...
for tt = 1 : 5*timeLagM
    ...
    meanBC1 = mean(bMagVariable_out_C100);
    meanBC2 = mean(bMagVariable_out_C200);
    covSum = 0;
    varBC1 = 0;
    varBC2 = 0;
    for ii = 1 : length(bMagVariable_out_C100)
        diffBC1 = bMagVariable_out_C100(ii) - meanBC1;
        diffBC2 = bMagVariable_out_C200(ii) - meanBC2;
        covSum = covSum + diffBC1 * diffBC2;
        varBC1 = varBC1 + diffBC1 ^ 2;
        varBC2 = varBC2 + diffBC2 ^ 2;
    end
    p21(tt) = covSum/sqrt(varBC1 * varBC2);
    end
end%%tt = 1 : 5*timeLagM
p21Max = max(p21);
inP21Max = find(p21 == p21Max);
timeLag21 = (inP21Max - timeAhead*5)/5;
s2 = inBStartC1 - timeAhead*5 + inP21Max;
...
for tt = 1 : 5*timeLagM
    ...
    meanBC1 = mean(bMagVariable_out_C100);
    meanBC3 = mean(bMagVariable_out_C300);
    covSum = 0;
    varBC1 = 0;
    varBC3 = 0;
    for ii = 1 : length(bMagVariable_out_C100)
        diffBC1 = bMagVariable_out_C100(ii) - meanBC1;
        diffBC3 = bMagVariable_out_C300(ii) - meanBC3;
        covSum = covSum + diffBC1 * diffBC3;
        varBC1 = varBC1 + diffBC1 ^ 2;
        varBC3 = varBC3 + diffBC3 ^ 2;
    end
    p31(tt) = covSum/sqrt(varBC1 * varBC3);
    end
end%%tt = 1 : 5*timeLagM
p31Max = max(p31);
inP31Max = find(p31 == p31Max);
timeLag31 = (inP31Max - timeAhead*5)/5;
s3 = inBStartC1 - timeAhead*5 + inP31Max;
...
for tt = 1 : 5*timeLagM
    ...
    meanBC1 = mean(bMagVariable_out_C100);
    meanBC4 = mean(bMagVariable_out_C400);
    covSum = 0;
    varBC1 = 0;
    varBC4 = 0;
    for ii = 1 : length(bMagVariable_out_C100)
        diffBC1 = bMagVariable_out_C100(ii) - meanBC1;
        diffBC4 = bMagVariable_out_C400(ii) - meanBC4;
        covSum = covSum + diffBC1 * diffBC4;
        varBC1 = varBC1 + diffBC1 ^ 2;
        varBC4 = varBC4 + diffBC4 ^ 2;
    end
    p41(tt) = covSum/sqrt(varBC1 * varBC4);
    end
end%%tt = 1 : 5*timeLagM
p41Max = max(p41);
inP41Max = find(p41 == p41Max);
timeLag41 = (inP41Max - timeAhead*5)/5;
s4 = inBStartC1 - timeAhead*5 + inP41Max;
...
tis = sprintf('Cluster FGM 5VPS: %4.4d-%2.2d-%2.2d by PP', beginYear, beginMonth, beginDate);
tit1 = title(tis);
...
xmin3=min(min(x3));
xmax3=max(max(x3));
text(xmin3+0.015*(xmax3-xmin3),ymin3+0.8*(ymax3-ymin3),['Time delay (C2 - C1): ', num2str(timeLag21, '%9.1f'),'s; p21: ',num2str(p21Max,'%9.4f')],'fontsize',12);
text(xmin3+0.015*(xmax3-xmin3),ymin3+0.5*(ymax3-ymin3),['Time delay (C3 - C1): ', num2str(timeLag31, '%9.1f'),'s; p31: ',num2str(p31Max,'%9.4f')],'fontsize',12);
text(xmin3+0.015*(xmax3-xmin3),ymin3+0.2*(ymax3-ymin3),['Time delay (C4 - C1): ', num2str(timeLag41, '%9.1f'),'s; p41: ',num2str(p41Max,'%9.4f')],'fontsize',12);
...
%
dir_name=sprintf('TimeDelayTiming_C1234PP');
ifexist=dir([pathOut,dir_name]);
if length(ifexist)==0
    mkdir(pathOut,dir_name);
end
picname=sprintf('%s%s\\PP5VPSDataCavitonVelocityInSolarWindFrame_%s%s%s_GSE_%s%s%s-%s%s%s',...
