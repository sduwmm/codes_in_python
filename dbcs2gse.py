    ...
    for ii in loopEner:
        ...
        for jj in loopTheta:
            ...
            for kk in loopPhi:
                ...
                nByVxTT += deltaTheta * deltaPhi * velIICube * velDelII * (math.sin(thetaJJ)) ** 2 * math.cos(phiKK) * \
                           distTT[ii, jj, kk]
                nByVyTT += deltaTheta * deltaPhi * velIICube * velDelII * (math.sin(thetaJJ)) ** 2 * math.sin(phiKK) * \
                           distTT[ii, jj, kk]
                nByVzTT += deltaTheta * deltaPhi * velIICube * velDelII * math.sin(thetaJJ) * math.cos(thetaJJ) * \
                           distTT[ii, jj, kk]
...
vDBCS = np.stack((vxKMS * (-1), vyKMS * (-1), vzKMS * (-1)))
...
quatECI2DBCSArrToCalNew = np.stack(
    (quatECI2DBCSArrToCalW, quatECI2DBCSArrToCalX, quatECI2DBCSArrToCalY, quatECI2DBCSArrToCalZ))
quatECI2DBCSArrToCalNew = np.transpose(quatECI2DBCSArrToCalNew)
...
quatECI2GSEArrToCalNew = np.stack(
    (quatECI2GSEArrToCalW, quatEIC2GSEArrToCalX, quatEIC2GSEArrToCalY, quatEIC2GSEArrToCalZ))
quatECI2GSEArrToCalNew = np.transpose(quatECI2GSEArrToCalNew)
