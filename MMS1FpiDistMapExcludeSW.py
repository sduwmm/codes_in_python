...
Modified on Tuesday Mar 16 17:04:00 2021
...
TimeInput = [2019, 2, 23, 5, 22, 9, 670]
FileName = \
...
TimeInTpl = np.where(Epoch >= EpochToCalType2000)####shouled be optimizeed
TimeInArr = TimeInTpl[0]
TimeIndex = TimeInArr[0]

PhiInput = np.arange(1,1,1)#[0, 1, 2,29,30,31]
ThetaInput = np.arange(3,3,1)
...
for ee in np.arange(32):
    ...
    #
    DistTimeEner[:, PhiInput] = 0  # added by wmm on 2021-03-16
    DistTimeEner[ThetaInput, :] = 0 #added by wmm on 2021-03-16
    #
    EnergyIndex = ee
    EnergyPlot = Energy[TimeIndex, ee]
    EnergyDeltaPlot = EnergyDelta[TimeIndex, ee]
    ...
    FigureName = FigurePath + 'MMS1_'+ str(TimeInput[0]) + str(TimeInput[1]) + str(TimeInput[2]) + \
